...
def forcast_single():
  ...
  try:
    ...
    f.save(secure_filename(file_name))
    # get file
    df = read_csv(file_name, header=0, parse_dates=[0], index_col=0, squeeze=True, date_parser=parser)
    data = df.values
...
# Forecast with no files uploaded
@app.route('/sales/multi', methods=['POST'])
def forcast_multi():
  # get file uploaded
  try:
    f = request.files['file']
